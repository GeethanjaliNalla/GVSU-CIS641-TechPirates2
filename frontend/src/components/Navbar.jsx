          ...
          if (token) {
            try {
              const response = await axios.post(
                backendUrl+'/api/user/profile-detail',
                { token: token },
                { headers: { token } }
                // Send the token in headers
              );
              if (response.data.success) {
                setUserProfile(response.data.user); // Update user profile state
              } else {
  ...
  return (
    <div className='flex items-center justify-between py-5 font-medium'>
      ...
      <div className='flex items-center gap-6'>
        ...
        {showProfileModal && (
        <div
          onClick={() => setShowProfileModal(false)} // Close modal on click outside
          className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50"
        >
          <div
            onClick={(e) => e.stopPropagation()} // Prevent closing when clicking inside the modal
            className="bg-white p-6 rounded-lg w-[300px] shadow-md"
          >
